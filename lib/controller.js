		...
		if(config.json != undefined){
			...
			this.render_template(function(yield){
				...
				if(config.layout !== false){
					_.extend(controller.view, {yield: yield});
	...
	render_layout: function(layout_name, complete){
		...
		async.glob(layout_glob).toArray(function(err, files){
			if(files.length < 1){
				...
				layout = files[0].name
				Mu.root = layout_path;
				//console.log("Rendering View: ", Mu.root, layout, controller.view);
				
				Mu.compile(layout, function (err, compiled_template) {
			    if (err) throw err;

					Mu.render(compiled_template, controller.view)
	...
	render_template: function(complete){
		...
		async.glob(template_glob).toArray(function(err, files){
			if(files.length < 1){
				...
				//console.log("Rendering View: ", Mu.root, template, controller.view);
